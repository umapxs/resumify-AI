                ...
                <div class="bg-gray-200 bg-opacity-25 p-6 lg:p-8">
                    <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
                            <form method="POST" action="{{ route('resume.store', ['redirect' => 'dashboard']) }}">
                                @csrf
                                            ...
                                            <div>
                                                ...
                                                <div class="mt-6 relative">
                                                    <textarea
                                                        ...
                                                        rows="4"
                                                        maxlength="1000"
                                                        placeholder="What would you like to resumify?">{{ session('input_text') }}</textarea>
                                                        <p id="char_count" class="text-sm text-black mt-2 bottom-0 left-0">0/1000 Characters</p>
                                                </div>
                                            </div>
                                            </div>
                                        </div>
                            ...
                            <div class="bg-white px-4 py-5 bg-white sm:p-6 shadow sm:rounded-tl-md sm:rounded-tr-md">
                                <h2 class="block font-medium text-md text-gray-700 mb-6">Result</h2>
                                <hr class="mb-6">
                                @if(session()->has('summarized'))
                                    <p class="font-medium text-green-500">{{ session('summarized') }}</p>
                                @endif
                            </div>
                        </div>
